...
class ActivityIndicatorLoader {
    ...
    static func startActivityIndicator(view: UIView) -> NVActivityIndicatorView {
        let cellWidth = Int(view.frame.width / CGFloat(5))
        let cellHeight = Int(view.frame.height / CGFloat(10))
        let x = Int(view.frame.width / 2) - cellWidth / 2
        let y = Int(view.frame.height / 2) - cellWidth / 2
        let frame = CGRect(x: x, y: y, width: cellWidth, height: cellHeight)
        let activityIndicator = NVActivityIndicatorView(frame: frame, type: .ballClipRotate, color: UIColor.flatWatermelon, padding: nil)
